  ...
  watch: {
    ...
    value(newVal) {
      if (newVal !== this.selectedValue) {
        // 默认值必须为undefined
        this.selectedValue = newVal ? newVal : undefined
      }
    },
  },
  async mounted() {
  ...
  methods: {
    ...
    change(vals) {
      ...
      this.dataSet.forEach((item, index) => {
        if (vals.includes(item[this.getField])) {
          checkedItems.push(item)
        }
      })
      const texts = checkedItems.map((item) => {return item[this.showField]})
      this.$emit('changeData', vals, texts, checkedItems, this.dataSet)
      this.$emit('input', vals) // 推给input组件处理空字串的问题
    }
  }
}
</script>
