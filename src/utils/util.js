...
export function logout() {
  ...
  // 默认登出后，再登陆就指向首页
  let path = '/'
  // 如果 401|403 的登出，后面携带的from参数
  if (/\/(401|403)/.test(router.currentRoute.fullPath)) {
    if (router.currentRoute.query.from) {
      path = router.currentRoute.query.from
    }
  } else if (/[\/%2F]empMessage/i.test(router.currentRoute.fullPath)) {
    // empMessage 特殊处理
    // 1. http://xxxx.com/empMessage/123333 #这种情况需要记录router.currentRoute.fullPath
    // 2. http://xxxx.com/401?from=%2FempMessage%2F123333 #这种情况已经被 401 | 403 条件分支处理
    // 2. http://xxxx.com/login?to=%2FempMessage%2F123333 #这种情况需要记录 router.currentRoute.query.to
    if (/from=%2FempMessage/i.test(router.currentRoute.fullPath)) {
      path = router.currentRoute.query.from
    } else if (/to=%2FempMessage/i.test(router.currentRoute.fullPath)) {
      path = router.currentRoute.query.to
    } else if (/\/empMessage/i.test(router.currentRoute.fullPath)) {
      path = router.currentRoute.fullPath
    }
  }
  window.location.replace(`${process.env.VUE_APP_LOGOUT_URL}${encodeURIComponent(path)}`)
}
