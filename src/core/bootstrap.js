import Vue from 'vue'
import store from '@/store/'
import {
  LS_VERSION,
  ACCESS_TOKEN,
  DEFAULT_COLOR,
  DEFAULT_THEME,
  DEFAULT_LAYOUT_MODE,
  DEFAULT_COLOR_WEAK,
  SIDEBAR_TYPE,
  DEFAULT_FIXED_HEADER,
  DEFAULT_FIXED_HEADER_HIDDEN,
  DEFAULT_FIXED_SIDEMENU,
  DEFAULT_CONTENT_WIDTH_TYPE,
  DEFAULT_MULTI_TAB
} from '@/store/mutation-types'
import config from '@/config/defaultSettings'

export default function Initializer () {
  const clientVersion = Vue.ls.get(LS_VERSION)
  const configVersion = config.version
  console.log('config:', config)
  if (clientVersion === configVersion) {
    store.commit('SET_LS_VERSION', clientVersion)
    store.commit('SET_SIDEBAR_TYPE', Vue.ls.get(SIDEBAR_TYPE))
    store.commit('TOGGLE_THEME', Vue.ls.get(DEFAULT_THEME))
    store.commit('TOGGLE_LAYOUT_MODE', Vue.ls.get(DEFAULT_LAYOUT_MODE))
    store.commit('TOGGLE_FIXED_HEADER', Vue.ls.get(DEFAULT_FIXED_HEADER))
    store.commit('TOGGLE_FIXED_SIDERBAR', Vue.ls.get(DEFAULT_FIXED_SIDEMENU))
    store.commit('TOGGLE_CONTENT_WIDTH', Vue.ls.get(DEFAULT_CONTENT_WIDTH_TYPE))
    store.commit('TOGGLE_FIXED_HEADER_HIDDEN', Vue.ls.get(DEFAULT_FIXED_HEADER_HIDDEN))
    store.commit('TOGGLE_WEAK', Vue.ls.get(DEFAULT_COLOR_WEAK))
    store.commit('TOGGLE_COLOR', Vue.ls.get(DEFAULT_COLOR))
    store.commit('TOGGLE_MULTI_TAB', Vue.ls.get(DEFAULT_MULTI_TAB))
  } else {
    store.commit('SET_LS_VERSION', configVersion)
    store.commit('SET_SIDEBAR_TYPE', true)
    store.commit('TOGGLE_THEME', config.navTheme)
    store.commit('TOGGLE_LAYOUT_MODE', config.layout)
    store.commit('TOGGLE_FIXED_HEADER', config.fixedHeader)
    store.commit('TOGGLE_FIXED_SIDERBAR', config.fixSiderbar)
    store.commit('TOGGLE_CONTENT_WIDTH', config.contentWidth)
    store.commit('TOGGLE_FIXED_HEADER_HIDDEN', config.autoHideHeader)
    store.commit('TOGGLE_WEAK', config.colorWeak)
    store.commit('TOGGLE_COLOR', config.primaryColor)
    store.commit('TOGGLE_MULTI_TAB', config.multiTab)
  }

  store.commit('SET_TOKEN', Vue.ls.get(ACCESS_TOKEN))
  console.log(`API_URL: ${process.env.VUE_APP_API_BASE_URL}`)
  // last step
}
